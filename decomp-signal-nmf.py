...
# ==========================================
# 2. ALGORITHMES MATHÉMATIQUES (NMF)
# ==========================================

def preprocess_for_nmf(sig, fs=2048.0, f0=50.0):
    """ Filtres Physiologiques + Redressement (Obligatoire pour NMF) """
    ...
    # Redressement absolu (NMF ne supporte pas les valeurs négatives)
    x_rectified = np.abs(x_filt)
    return x_rectified

def NMFdecomp(sig_rectified, n_components):
    """ Décomposition par NMF """
    nmf = NMF(n_components=n_components, init='nndsvda', random_state=42, max_iter=500)
    W = nmf.fit_transform(sig_rectified.T) 
    H = nmf.components_
    
    sources = W.T # shape (n_components, n_samples)
    
    # Tableau d'autocorrélation virtuel pour maintenir la compatibilité de l'interface
    pseudo_autocor = np.ones(n_components) 
    
    return sources, H, pseudo_autocor

def detect_spikes_kmeans(source, fs=2048.0, min_distance_ms=25.0):
    """ Binarisation K-Means avec calcul du PNR """
    signal_sq = source ** 2
...
class NMFMainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("NMF Explorer - Synergies & Pics d'Activation")
    ...
    def initUI(self):
        ...
        self.btn_run_nmf = QPushButton("⚡ Décomposition NMF")
        self.btn_run_nmf.clicked.connect(self.run_nmf)
        self.btn_run_nmf.setEnabled(False)
        self.btn_run_nmf.setStyleSheet("font-weight: bold;")
        
        lbl_comp = QLabel("Nb Composantes :")
        self.spin_comp = QSpinBox()
        self.spin_comp.setRange(1, 100)
        self.spin_comp.setValue(4) 
        ...
        layout_l1.addWidget(self.btn_run_nmf)
        layout_l1.addWidget(lbl_comp)
        layout_l1.addWidget(self.spin_comp)
        ...
        self.btn_spikes = QPushButton("🎯 Extraire Pics (K-Means)")
        ...
        self.btn_export_sources = QPushButton("📈 Exporter Sources NMF")
        ...
        self.btn_export_spikes = QPushButton("📥 Exporter Train de Pics (0 et 1)")
        ...
        self.lbl_mu_count = QLabel("Synergies Validées : <b>-</b>")
        ...
        self.plot_raw = pg.PlotWidget(title="Signaux sEMG Redressés (Entrée NMF)")
        main_layout.addWidget(self.plot_raw)
        self.plot_sources = pg.PlotWidget(title="Sources NMF Validées & Pics détectés")
    ...
    def open_file(self):
        ...
        try:
            ...
            if best_data is not None:
                ...
                self.btn_run_nmf.setEnabled(True)
                
                # Prétraitement et redressement du signal complet
                sig_rect = preprocess_for_nmf(self.signal, fs=self.spin_fs.value())
                
                self.plot_raw.clear()
                self.plot_sources.clear()
                offset = np.max(sig_rect) * 1.5
                
                # MODIFICATION ICI : Affiche la totalité des canaux (ex: les 64 canaux)
                for i in range(sig_rect.shape[0]): 
                    self.plot_raw.plot(sig_rect[i] + (i * offset), pen='#3498db')
    ...
    def run_nmf(self):
        ...
        try:
            # 1. Filtre Physiologique + Redressement absolu
            sig_rect = preprocess_for_nmf(self.signal, fs=self.spin_fs.value())
            
            # 2. Décomposition NMF
            self.sources, w_x, self.autocor = NMFdecomp(sig_rect, self.spin_comp.value())
    ...
    def run_spike_detection(self):
        ...
        for i in range(self.sources.shape[0]):
            ...
            if len(peaks) >= 5 and mu_spec >= self.spin_spec.value() and pnr >= self.spin_pnr.value():
                isi = np.diff(peaks) 
        ...
        self.lbl_mu_count.setText(f"Synergies Validées : <b>{mu_count}</b>")
        if mu_count > 0: 
    ...
    def export_spikes_csv(self):
        ...
        options = QFileDialog.Options()
        file_path, _ = QFileDialog.getSaveFileName(self, "Exporter Train de Pics", "", "Fichier CSV (*.csv)", options=options)
